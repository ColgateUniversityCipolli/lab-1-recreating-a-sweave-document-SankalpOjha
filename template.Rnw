...
The result is \Sexpr{round(result.with.error, 5)}. Note that I did not type the result, but I used the \verb|\Sexpr{}| command.
...
The \texttt{plamerpenguins} package for \texttt{R} \citep{palmerP} provides data on adult foraging penguins near Palmer Station, Antarctica. Figure \hyperref[plot5]{2} is too big to fit nicely in our column-based-template above, so I've placed it here in the abstract by saving it and presenting it scaled to 0.75.

<<plot5, eval = FALSE, size='scriptsize'>>=
